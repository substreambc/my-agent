...
Helium Network, Solana native with over 2,000,000 txs and 1 year history.

Pay-per-query intelligence for DePIN infrastructure via MCP and the `helium-mcp` onboarding helper. *(Note: Live real-time stream access is available upon request; the primary interface is the historical datalake).*
...
**x402 Settlement Tiers (Strict `LIMIT 1` per query):**
*   **Info / Low:** $0.01 USDC
*   **Medium:** $0.50 USDC
*   **High:** $0.75 USDC
*   **Critical / RF Physics Data:** $1.00 USDC
...
## Quick start — free, no payment, no key needed
...
code
Bash
# 1. request → 402 with payment requirements (Strict LIMIT 1 enforced)
...
Settlement proof	2TEnsbpux…7J939c — live agent-to-agent settlement, verify it yourself
Discovery
Surface	URL
...
The 36+ Skills (Sorted by Value / Price Descending)
...
Repo: substreambc/helium-mcp · npm: @web3solutions33/helium-mcp · MIT
Response shape
code
JSON
{
  ...
  "rowCount": 1,
  "rows": [ { "hotspot_address": "...", "rssi_delta_dbm": -42.5, "physics_violation": true, "transmit_scale": 0.15 } ]
}
Notes
